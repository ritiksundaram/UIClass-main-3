...
function CapacityMessage(currentCapacity) {
  ...
  if (parsed >= 0 && parsed <= 25) {
    return "Estimated 6-8 lanes available";
  }
  else if (parsed >= 26 && parsed <= 50) {
    return "Estimated 4-6 lanes available";
  }
  else if (parsed >= 51 && parsed <= 75) {
    return "Estimated 2-5 lanes available";
  }
  else if (parsed >= 76) {
    return "Estimated 1 lane available";
  }
}
  ...
  useEffect(() => {
    fetch("/api/data").then(
      ...
      data => {
        ...
        const capacityData = data.LOCATIONS["Pool"];

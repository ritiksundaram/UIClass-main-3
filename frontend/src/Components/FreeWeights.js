...
function CapacityMessage(currentCapacity) {
  ...
  if (parsed >= 0 && parsed <= 25) {
    return "Estimated no wait for a bench";
  }
  else if (parsed >= 26 && parsed <= 50) {
    return "Estimated 5 minute wait for a bench";
  }
  else if (parsed >= 51 && parsed <= 75) {
    return "Estimated 10 minute wait for a bench";
  }
  else if (parsed >= 76) {
    return "Estimated 15 minute wait for a bench";
  }
}

const FreeWeights = ({ onReset }) => {
  ...
  useEffect(() => {
    fetch("/api/data").then(
      ...
      data => {
        ...
        const capacityData = data.LOCATIONS["Free Weights"];
  ...
  return (
    <div class="info">
      <span class="bolded"> Free Weights: </span>
